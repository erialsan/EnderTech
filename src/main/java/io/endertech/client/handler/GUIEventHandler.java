...
public class GUIEventHandler extends Gui {
    ...
    public void onGameOverlayRender(RenderGameOverlayEvent event) {
        ...
        if (player.inventory.getCurrentItem() == null || !(player.inventory.getCurrentItem()
            .getItem() instanceof ItemExchanger)) return;

        ItemExchanger exchanger = (ItemExchanger) player.inventory.getCurrentItem()
            .getItem();
        ...
        if (this.lastExchangeSource.getItem() != null) {
            ...
            ri.renderItemAndEffectIntoGUI(
                mc.fontRenderer,
                mc.renderEngine,
                this.lastExchangeSource,
                2 + GeneralConfig.GUITopLeftXOffset,
                2 + GeneralConfig.GUITopLeftYOffset);
        ...
        FontHelper.drawItemQuantity(
            mc.fontRenderer,
            3 + GeneralConfig.GUITopLeftXOffset,
            3 + GeneralConfig.GUITopLeftYOffset,
            am);
        FontHelper.renderText(
            mc.fontRenderer,
            2 + 16 + 2 + GeneralConfig.GUITopLeftXOffset,
            3 + GeneralConfig.GUITopLeftYOffset,
            1.0,
            LocalisationHelper.localiseString("gui.exchanger.radius", exchanger.getTargetRadius(exchangerStack)));
