...
public class WorldEventHandler {
    ...
    private void checkAndPerformExchanges(Set<Exchange> queue, World world) {
        ...
        for (Exchange exchange : queue) {
            ...
            if (exchangerStack == null || exchangerStack.getItem() == null
                || !(exchangerStack.getItem() instanceof ItemExchanger)) cullExchange = true;
            ...
            Set<BlockCoord> blocks = Geometry
                .squareSet(exchange.currentRadius - 1, exchange.origin, exchange.orientation);
            boolean stop = false;
            for (BlockCoord blockCoord : blocks) {
                if (stop) break;
                ExchangeResult result = checkAndPerformExchange(
                    exchange,
                    exchanger,
                    exchangerStack,
                    world,
                    new BlockCoord(blockCoord.x, blockCoord.y, blockCoord.z));
                switch (result) {
                    case FAIL_ENERGY:
        ...
        for (Exchange removal : removals) queue.remove(removal);
    }
    ...
    private ExchangeResult checkAndPerformExchange(Exchange exchange, ItemExchanger exchanger, ItemStack exchangerStack,
        World world, BlockCoord blockCoord) {
        Block block = world.getBlock(blockCoord.x, blockCoord.y, blockCoord.z);

        if (!Exchange.blockSuitableForExchange(
            blockCoord,
            world,
            exchange.source,
            exchange.sourceMeta,
            exchange.target,
            exchangerStack,
            exchange.currentRadius - 1)) return ExchangeResult.FAIL_BLOCK_NOT_REPLACEABLE;

        int exchangeCost = this.calculateExchangeCost(exchange, block, world, blockCoord);
        if (exchanger.extractEnergy(exchange.player.inventory.getStackInSlot(exchange.hotbar_id), exchangeCost, true)
            < exchangeCost) return ExchangeResult.FAIL_ENERGY;
        ...
        if (!isCreativeExchanger) {
            ...
            if (beforeConsumedSize == 1 && (exchange.player instanceof FakePlayer)) {
                LogHelper.debug(
                    "Not consuming item as it is the last in a Fake Player's stack: "
                        + exchange.player.getPosition(1.0F));
            ...
            if (droppedItems == null) {
                LogHelper.warn(
                    "Player " + exchange.player.getDisplayName()
                        + " tried to exchange something, but the droppedItems were null - bailing the entire exchange: "
                        + block.toString());
            ...
            for (ItemStack droppedItem : droppedItems) {
                if (droppedItem.getItem() == null) {
                    LogHelper.warn(
                        "Player " + exchange.player.getDisplayName()
                            + " tried to exchange a null item - bailing the entire exchange: "
            ...
            boolean canPutItemsInInventory = InventoryHelper
                .canPutItemStacksInToInventory(exchange.player.inventory, droppedItems);
            if (!canPutItemsInInventory) {
                return ExchangeResult.FAIL_INVENTORY_SPACE;
            }

            boolean didPutItemsInInventory = InventoryHelper
                .checkAndPutItemStacksInToInventory(exchange.player.inventory, droppedItems);
    ...
    private List<ItemStack> harvestBlockWithSilkTouchIfRequired(Block block, Exchange exchange, BlockCoord blockCoord) {
        boolean canSilkTouch = block.canSilkHarvest(
            exchange.player.worldObj,
            exchange.player,
            blockCoord.x,
            blockCoord.y,
            blockCoord.z,
        ...
        if (canSilkTouch && ItemConfig.itemExchangerSilkTouch) {
            ...
            return block
                .getDrops(exchange.player.worldObj, blockCoord.x, blockCoord.y, blockCoord.z, exchange.sourceMeta, 0);
        }
    }
    ...
    private void performExchange(Exchange exchange, BlockCoord blockCoord, ItemExchanger exchanger, World world,
        ...
        world.setBlock(
            ...
            blockCoord.y,
            blockCoord.z,
            Block.getBlockFromItem(exchange.target.getItem()),
            exchange.target.getItemDamage(),
            3);
        ...
        world.playAuxSFX(
            ...
            blockCoord.y,
            blockCoord.z,
            Block.getIdFromBlock(exchange.source) + (exchange.sourceMeta << 12));
    }
