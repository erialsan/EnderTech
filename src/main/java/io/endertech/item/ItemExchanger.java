...
public class ItemExchanger extends ItemExchangerBase implements IKeyHandler, IOutlineDrawer, IItemBlockAffector {
        ...
        if (KeyHelper.isShiftDown()) {
            ...
            if (this.getMaxReceiveRate(stack) > 0) {
                list.add(
                    LocalisationHelper.localiseString(
                        "info.charge.receive",
                        StringHelper.getEnergyString(this.getMaxReceiveRate(stack)) + " RF/t"));
            }
            ...
            if (pb == null) {
                ...
                list.add(
                    EnumChatFormatting.GREEN
                        + LocalisationHelper.localiseString("info.exchanger.source", pb.getDisplayName())
                        + EnumChatFormatting.RESET);
            }

            list.add(
                EnumChatFormatting.GREEN
                    + LocalisationHelper.localiseString("info.exchanger.radius", this.getTargetRadius(stack)));

            list.add(
                EnumChatFormatting.AQUA + ""
                    + EnumChatFormatting.ITALIC
                    + LocalisationHelper.localiseString("info.exchanger.shift_to_select_source")
                    + EnumChatFormatting.RESET);
        } else {
    ...
    public boolean onItemUse(ItemStack itemstack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if ((pb != null) && (player.worldObj.getTileEntity(x, y, z) == null) && !player.worldObj.isRemote) {
            ...
            if (Exchange.blockSuitableForExchange(coord, world, source, meta, pb, itemstack, 0))
                WorldEventHandler.queueExchangeRequest(
                    player.worldObj,
                    coord,
                    this.getTargetRadius(itemstack),
                    source,
                    meta,
                    pb,
                    player,
            ...
            // TeleportHelper.teleportPlayerToDimensionWithCoords((EntityPlayerMP) player, player.dimension,
            // player.posX, player.posY + 10, player.posZ);
        }
    ...
    public void setTargetRadius(ItemStack stack, int radius) {
        ...
        stack.getTagCompound()
            .setInteger("targetRadius", radius);
    }
    ...
    public int getTargetRadius(ItemStack stack) {
        ...
        if (stack.hasTagCompound() && (stack.getTagCompound()
            .hasKey("targetRadius"))) {
            radius = stack.getTagCompound()
                .getInteger("targetRadius");
        }
    ...
    public boolean drawOutline(DrawBlockHighlightEvent event) {
        ...
        if (event.player.isSneaking()) {
            ...
            if (!Exchange
                .blockSuitableForSelection(target, world, block, blockMeta, event.player.getCurrentEquippedItem()))
                RenderHelper.renderBlockOutline(
                    event.context,
                    event.player,
                    target,
                    RGBA.Red.setAlpha(0.6f),
                    2.0f,
                    event.partialTicks);
            else RenderHelper.renderBlockOutline(
                event.context,
                event.player,
                target,
                RGBA.Green.setAlpha(0.6f),
                2.0f,
                event.partialTicks);
        ...
        Set<BlockCoord> blocks = this
            .blocksAffected(event.currentItem, world, target, ForgeDirection.getOrientation(event.target.sideHit));
        ...
        for (int radius = 0; radius <= exchangerRadius; radius++) {
            ...
            for (BlockCoord blockCoord : squareSet) {
                if (Exchange.blockSuitableForExchange(
                    ...
                    targetBlock,
                    targetMeta,
                    this.getSourceItemStack(item),
                    item,
                    radius)) ret.add(blockCoord);
            }
        }
