...
public abstract class MultiblockControllerBase implements IOutlineDrawer, ITilePacketHandler {
        ...
        if (!connectedParts.add(part)) {
            LogHelper.warn(
                LocalisationHelper.localiseString(
                    "warning.multiblock.part.double_adding",
    ...
    public void detachBlock(IMultiblockPart part, boolean chunkUnloading) {
        ...
        if (!connectedParts.remove(part)) {
            LogHelper.warn(
                LocalisationHelper.localiseString(
                    "warning.multiblock.part.double_removing",
                    worldObj.isRemote ? "CLIENT" : "SERVER",
                    part.hashCode(),
                    part.xCoord,
                    part.yCoord,
            ...
            if (minimumCoord != null && maximumCoord != null
                ...
                for (int x = minChunkX; x <= maxChunkX; x++) {
                    for (int z = minChunkZ; z <= maxChunkZ; z++) {
                        // Ensure that we save our data, even if the chunk our save delegate is in has no TEs.
    ...
    protected void isBlockGoodForFrame(World world, int x, int y, int z) throws MultiblockValidationException {
        throw new MultiblockValidationException(
            LocalisationHelper.localiseString("info.multiblock.part.unsuitable.frame", x, y, z));
    }
    ...
    protected void isBlockGoodForTop(World world, int x, int y, int z) throws MultiblockValidationException {
        throw new MultiblockValidationException(
            LocalisationHelper.localiseString("info.multiblock.part.unsuitable.top", x, y, z));
    }
    ...
    protected void isBlockGoodForBottom(World world, int x, int y, int z) throws MultiblockValidationException {
        throw new MultiblockValidationException(
            LocalisationHelper.localiseString("info.multiblock.part.unsuitable.bottom", x, y, z));
    }
    ...
    protected void isBlockGoodForSides(World world, int x, int y, int z) throws MultiblockValidationException {
        throw new MultiblockValidationException(
            LocalisationHelper.localiseString("info.multiblock.part.unsuitable.sides", x, y, z));
    }
    ...
    protected void isBlockGoodForInterior(World world, int x, int y, int z) throws MultiblockValidationException {
        throw new MultiblockValidationException(
            LocalisationHelper.localiseString("info.multiblock.part.unsuitable.interior", x, y, z));
    }
    ...
    public void recalculateMinMaxCoords() {
        minimumCoord = new BlockCoord(Integer.MAX_VALUE, Integer.MAX_VALUE, Integer.MAX_VALUE);
        maximumCoord = new BlockCoord(Integer.MIN_VALUE, Integer.MIN_VALUE, Integer.MIN_VALUE);
    ...
    public boolean shouldConsume(MultiblockControllerBase otherController) {
        if (!otherController.getClass()
            .equals(getClass())) {
            throw new IllegalArgumentException(
                LocalisationHelper.localiseString("error.multiblock.consuming.different_classes"));
        }
        ...
        if (res < 0) {
            ...
            LogHelper.warn(
                LocalisationHelper.localiseString(
                    "warning.multiblock.consuming.same_reference_coordinate",
                    worldObj.isRemote ? "CLIENT" : "SERVER"));
            auditParts();
            ...
            if (res < 0) {
                ...
                LogHelper.fatal(
                    LocalisationHelper.localiseString(
                        "error.multiblock.consuming.my_controller",
                        hashCode(),
                        connectedParts.size(),
                        getPartsListString()));
                LogHelper.fatal(
                    LocalisationHelper.localiseString(
                        "error.multiblock.consuming.other_controller",
                        otherController.hashCode(),
                        otherController.connectedParts.size(),
                        otherController.getPartsListString()));
                throw new IllegalArgumentException(
                    LocalisationHelper.localiseString(
                        "error.multiblock.consuming.same_reference_coordinate",
                        (worldObj.isRemote ? "CLIENT" : "SERVER")));
            }
    ...
    private int _shouldConsume(MultiblockControllerBase otherController) {
        BlockCoord myCoord = getReferenceCoord();
