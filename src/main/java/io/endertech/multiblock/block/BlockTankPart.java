...
public class BlockTankPart extends BlockET implements ITileEntityProvider, IOutlineDrawer, IDismantleable {
    ...
    private static String[] _subBlocks = new String[] { "frameDefault", "frameCorner", "frameCenter", "frameVertical",
        "frameEastWest", "frameNorthSouth", "valve", "energyInput" };
    ...
    public static boolean isFrame(int metadata) {
        return metadata >= FRAME_METADATA_BASE && metadata <= FRAME_NORTHSOUTH;
    }

    public static boolean isValve(int metadata) {
        return metadata == VALVE_BASE;
    }

    public static boolean isEnergyInput(int metadata) {
        return metadata == ENERGY_INPUT_BASE;
    }

    public static boolean onTankBlockActivated(World world, int x, int y, int z, EntityPlayer player, int faceHit,
                        ...
                        if (e != null) {
                            player.addChatComponentMessage(
                                new ChatComponentText(
                                    LocalisationHelper
                                        .localiseString("info.multiblock.construction.last_failure_reason.pre")));
                            player.addChatComponentMessage(new ChatComponentText(e.getMessage()));
                        }
                    } else {
                        player.addChatComponentMessage(
                            new ChatComponentText(
                                LocalisationHelper.localiseString("info.multiblock.construction.no_controller")));
                        return true;
                    }
                }
            }
                        ...
                        if (randomNumbers.size() > 1) {
                            LogHelper.info(
                                LocalisationHelper.localiseString(
                                    "info.multiblock.tank.connecting_destructive",
                                    new BlockCoord(x, y, z).toString()));
                            return false;
                        }
                    }
                }
    ...
    public static boolean isLastPartWithContents(World world, BlockCoord coord) {
        ...
        for (ForgeDirection direction : ForgeDirection.VALID_DIRECTIONS) {
            TileEntity neighbourTile = world
                .getTileEntity(coord.x + direction.offsetX, coord.y + direction.offsetY, coord.z + direction.offsetZ);
    ...
    public static boolean canDismantleTankBlock(EntityPlayer player, World world, int x, int y, int z) {
        ...
        if (dismantleWouldBeDestructive) player.addChatComponentMessage(
            new ChatComponentText(LocalisationHelper.localiseString("warning.tank.dismantle_loss")));
    ...
    public ItemStack getTankValveItemStack() {
        return new ItemStack(this, 1, VALVE_BASE);
    }

    public ItemStack getTankEnergyInputItemStack() {
        return new ItemStack(this, 1, ENERGY_INPUT_BASE);
    }
