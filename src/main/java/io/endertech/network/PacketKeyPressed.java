...
public class PacketKeyPressed extends PacketETBase {
    ...
    public void handlePacket(EntityPlayer entityPlayer, boolean isServer) {
        if (entityPlayer.getCurrentEquippedItem() != null && entityPlayer.getCurrentEquippedItem()
            .getItem() instanceof IKeyHandler) {
            ((IKeyHandler) entityPlayer.getCurrentEquippedItem()
                .getItem())
                    .handleKey(entityPlayer, entityPlayer.getCurrentEquippedItem(), Key.fromByte(this.getByte()));
        }
    }
