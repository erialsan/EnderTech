...
public class ClientProxy extends CommonProxy {
    ...
    public void registerItemRenderers() {
        MinecraftForgeClient
            .registerItemRenderer(Item.getItemFromBlock(ETBlocks.blockTankController), new TankControllerRenderer());
    }
    ...
    public void registerIcons(TextureStitchEvent.Pre event) {
        ...
        for (String type : types) {
            if (Loader.isModLoaded("ThermalExpansion")) {
                IconRegistry
                    .addAndRegisterIcon("Machine_Redstone_" + type, Textures.TE3_TEXTURE_BASE + type, event.map);
            } else {
                IconRegistry
                    .addAndRegisterIcon("Machine_Redstone_" + type, Textures.REDSTONE_TEXTURE_BASE + type, event.map);
            }
            IconRegistry
                .addAndRegisterIcon("Machine_Creative_" + type, Textures.CREATIVE_TEXTURE_BASE + type, event.map);
    ...
    public void initializeIcons(TextureStitchEvent.Post event) {
        ...
        // {
        // DevETFluids.fluidChargedEnder.setIcons(DevETFluids.fluidChargedEnderStill,
        // DevETFluids.fluidChargedEnderFlowing);
        // }
    }
}
