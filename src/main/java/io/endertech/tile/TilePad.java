...
public abstract class TilePad extends TileInventory
    ...
    public AxisAlignedBB getAABBInFront(int distance) {
        ...
        return AxisAlignedBB
            ...
            .expand(
                Math.abs(orientation.offsetX) * (distance - 1),
                Math.abs(orientation.offsetY) * (distance - 1),
                Math.abs(orientation.offsetZ) * (distance - 1))
            .offset(orientation.offsetX, orientation.offsetY, orientation.offsetZ);
    }
    ...
    public void chargeFromGUISlot() {
        ...
        int chargeAmount = Math
            .min(this.getMaxReceiveRate(meta), this.getMaxEnergyStored(meta) - this.getEnergyStored());
    ...
    public boolean drawOutline(DrawBlockHighlightEvent event) {
        if (GeneralConfig.debugRender) {
            ...
            RenderHelper.renderAABBOutline(
                event.context,
                event.player,
                front,
    ...
    public int receiveEnergy(int maxReceive, boolean simulate) {
        ...
        int energyReceived = Math.min(
            this.getMaxEnergyStored(blockMeta) - this.storedEnergy,
            Math.min(this.getMaxReceiveRate(blockMeta), maxReceive));
