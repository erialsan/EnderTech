...
public class BlockHelper {
    ...
    public static boolean isBlockExposed(World world, int x, int y, int z) {
        return isSoftBlock(world, x + 1, y, z) || isSoftBlock(world, x - 1, y, z)
            || isSoftBlock(world, x, y + 1, z)
            || isSoftBlock(world, x, y - 1, z)
            || isSoftBlock(world, x, y, z + 1)
            || isSoftBlock(world, x, y, z - 1);
    }
    ...
    public static boolean areBlocksEqual(IBlockAccess blockAccess, BlockCoord one, BlockCoord two) {
        return areBlocksEqual(
            blockAccess.getBlock(one.x, one.y, one.z),
            blockAccess.getBlockMetadata(one.x, one.y, one.z),
            blockAccess.getBlock(two.x, two.y, two.z),
            blockAccess.getBlockMetadata(two.x, two.y, two.z));
    }

    public static boolean areBlocksEqual(IBlockAccess blockAccess, int x1, int y1, int z1, int x2, int y2, int z2) {
        return areBlocksEqual(
            blockAccess.getBlock(x1, y1, z1),
            blockAccess.getBlockMetadata(x1, y1, z1),
            blockAccess.getBlock(x2, y2, z2),
            blockAccess.getBlockMetadata(x2, y2, z2));
    }
    ...
    public static boolean areBlocksEqual(IBlockAccess blockAccess, Block block, int meta, int x, int y, int z,
        ForgeDirection direction) {
        return (block == blockAccess.getBlock(x + direction.offsetX, y + direction.offsetY, z + direction.offsetZ)
            && meta
    ...
    public static List<ItemStack> createSilkTouchStack(Block block, int meta) {
        ...
        try {
            Method createStackedBlock = Block.class.getDeclaredMethod("func_149644_j", int.class); // #
                                                                                                   // createStackedBlock
            createStackedBlock.setAccessible(true);
            itemstack = (ItemStack) createStackedBlock.invoke(block, meta);
        } catch (NoSuchMethodException e) {} catch (IllegalArgumentException e) {} catch (IllegalAccessException e) {} catch (InvocationTargetException e) {}
