...
public interface IInventoryHandler extends IInventoryConnection {
    ...
    /**
     * Insert an ItemStack into the IInventoryHandler, internal distribution is left entirely to the IInventoryHandler.
     * This returns what is remaining of the
     ...
     * @return An ItemStack representing how much is remaining after the item was inserted (or would have been, if
     *         simulated) into the container inventory.
     */
    ItemStack insertItem(ForgeDirection from, ItemStack item, boolean simulate);

    /**
     * Extract an ItemStack from an IInventoryHandler, internal distribution is left entirely to the IInventoryHandler.
     * This returns the resulting stack - a
     * null return means that nothing was extracted!
     *
     * @param from
     *                 Orientation the item is extracted from.
     * @param item
     *                 ItemStack to be extracted. The size of this stack corresponds to the maximum amount to extract.
     *                 If this is null, then a null ItemStack should
     *                 immediately be returned.
     * @param simulate
     *                 If TRUE, the extraction will only be simulated.
     * @return An ItemStack representing how much was extracted (or would have been, if simulated) from the container
     *         inventory.
     */
     ...
     *                   Orientation the item is extracted from.
     * @param maxExtract
     *                   Maximum number of items to extract. (The returned ItemStack should have a stackSize no higher
     *                   than this.)
     * @param simulate
     *                   If TRUE, the extraction will only be simulated.
     * @return An ItemStack representing how much was extracted (or would have been, if simulated) from the container
    ...
    /**
     * Get the contents of the IInventoryHandler's inventory. This returns a COPY. This should only return non-null
     * ItemStacks, and an empty List if the
