...
public abstract class GuiBase extends GuiContainer {
    ...
    public static final SoundHandler guiSoundManager = FMLClientHandler.instance()
        .getClient()
        .getSoundHandler();
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        if (drawTitle) {
            fontRendererObj
                .drawString(StringHelper.localize(name), getCenteredOffset(StringHelper.localize(name)), 6, 0x404040);
        }
        if (drawInventory) {
            fontRendererObj
    ...
    public void handleMouseInput() {
        ...
        if (wheelMovement != 0) {
            ...
            if (onMouseWheel(mouseX, mouseY, wheelMovement)) return;
        }
    ...
    public TabBase addTab(TabBase tab) {
        ...
        int yOffset = 4;
        for (int i = 0; i < tabs.size(); i++) {
            if (tabs.get(i).side == tab.side && tabs.get(i)
                .isVisible()) {
        ...
        if (TabTracker.getOpenedLeftTab() != null && tab.getClass()
            .equals(TabTracker.getOpenedLeftTab())) {
            tab.setFullyOpen();
    ...
    public void drawItemStack(ItemStack stack, int x, int y, boolean drawOverlay, String overlayTxt) {
        ...
        if (drawOverlay) itemRender.renderItemOverlayIntoGUI(
            font,
            this.mc.getTextureManager(),
            stack,
            x,
            y - (this.draggedStack == null ? 0 : 8),
            overlayTxt);
