...
public class ElementListBox extends ElementBase {
    ...
    public boolean onMousePressed(int mouseX, int mouseY, int mouseButton) {
        ...
        for (int i = _firstIndexDisplayed; i < _elements.size(); i++) {
            ...
            int elementHeight = _elements.get(i)
                .getHeight();
            if (getContentTop() + heightChecked <= mouseY
                && getContentTop() + heightChecked + elementHeight >= mouseY) {
                setSelectedIndex(i);
    ...
    public void scrollDown() {
        ...
        for (int i = _firstIndexDisplayed; i < _elements.size(); i++) {
            if (heightDisplayed + _elements.get(i)
                .getHeight() > sizeY) {
                break;
            }
            heightDisplayed += _elements.get(i)
                .getHeight();
            elementsDisplayed++;
        }
    ...
    public int getLastScrollPosition() {
        ...
        while (position >= 0 && heightUsed < sizeY) {
            heightUsed += _elements.get(position--)
                .getHeight();
        }
        if (heightUsed > sizeY) {
            ++position;
