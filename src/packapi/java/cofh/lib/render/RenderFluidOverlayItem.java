...
public class RenderFluidOverlayItem implements IItemRenderer {
    ...
    protected void doRenderItem(ItemRenderType type, ItemStack item, Item iconItem, FluidStack fluid) {
        ...
        IIcon fluidIcon = hasFluid ? fluid.getFluid()
            .getIcon(fluid) : mask;
        int fluidSheet = hasFluid ? fluid.getFluid()
            .getSpriteNumber() : 0;
        int colorMult = hasFluid ? fluid.getFluid()
            .getColor(fluid) : 0xFFFFFF;
        boolean isFloaty = hasFluid ? fluid.getFluid()
            .getDensity(fluid) < 0 : false;
