...
public final class EnderRegistry {
    ...
    /* HELPER FUNCTIONS */
    private <T extends IEnderAttuned> boolean addHandler(HashMap<String, TIntObjectHashMap<ArrayHashList<T>>> map,
        T theAttuned) {
    ...
    public void addDestination(IEnderDestination theAttuned) {
        ...
        if (!hasDestination(theAttuned, false)) {
            ...
            map.put(freq, dest);
            linkConf.get(channel, String.valueOf(freq), "")
                .set(dest.toString());
        }
    }

    private <T> boolean removeHandler(HashMap<String, TIntObjectHashMap<ArrayHashList<T>>> map,
        IEnderAttuned theAttuned) {
    ...
    public void removeDestination(IEnderDestination theAttuned) {
        ...
        if (dest.dimension == theAttuned.dimension()) {
            if (dest.x == theAttuned.x() && dest.y == theAttuned.y() && dest.z == theAttuned.z()) {
                int freq = theAttuned.getFrequency();
                map.remove(freq);
                linkConf.getCategory(channel)
                    .remove(String.valueOf(freq));
            }
        }
    }
