...
public class UtilLiquidMover {
                ...
                if (!player.capabilities.isCreativeMode) {
                    if (item.hasContainerItem(ci)) {
                        ...
                        if (drop != null && drop.isItemStackDamageable()
                            && drop.getItemDamage() > drop.getMaxDamage()) {
                            drop = null;
                        }
                        ItemHelper.disposePlayerItem(ci, drop, player, true);
                    } else {
                        player.inventory
                            .setInventorySlotContents(player.inventory.currentItem, ItemHelper.consumeItem(ci, player));
                    }
    ...
    public static void pumpLiquid(IFluidTank iFluidTank, TileEntity from) {
        ...
        if (iFluidTank != null && iFluidTank.getFluidAmount() > 0) {
            FluidStack l = iFluidTank.getFluid()
                .copy();
