...
public class FluidHelper {
    ...
    public static FluidStack extractFluidFromAdjacentFluidHandler(TileEntity tile, int side, int maxDrain,
        boolean doDrain) {

        TileEntity handler = BlockHelper.getAdjacentTileEntity(tile, side);

        return handler instanceof IFluidHandler
            ? ((IFluidHandler) handler).drain(ForgeDirection.VALID_DIRECTIONS[side ^ 1], maxDrain, doDrain)
            : null;
    }
    ...
    public static int insertFluidIntoAdjacentFluidHandler(TileEntity tile, int side, FluidStack fluid, boolean doFill) {
        ...
        return handler instanceof IFluidHandler
            ? ((IFluidHandler) handler).fill(ForgeDirection.VALID_DIRECTIONS[side ^ 1], fluid, doFill)
            : 0;
    }
            ...
            if (!player.capabilities.isCreativeMode) {
                if (container.stackSize == 1) {
                    ...
                    if (ItemHelper.disposePlayerItem(player.getCurrentEquippedItem(), returnStack, player, true)) {
                        ...
                        ((EntityPlayerMP) player).sendContainerAndContentsToPlayer(
    ...
    public static FluidStack readFluidStackFromPacket(DataInput data) throws IOException {
        ...
        if (length < 0) {
            ...
            return FluidStack
                .loadFluidStackFromNBT(CompressedStreamTools.func_152457_a(abyte, new NBTSizeTracker(2097152L)));
        }
    }
