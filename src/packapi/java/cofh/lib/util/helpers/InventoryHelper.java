...
public class InventoryHelper {
        ...
        if (inventory instanceof ISidedInventory) {
            ...
            for (int i = 0; i < slots.length && retStack == null; i++) {
                if (sidedInv.getStackInSlot(i) != null
                    && sidedInv.canExtractItem(i, sidedInv.getStackInSlot(i), side)) {
                    retStack = sidedInv.getStackInSlot(i)
                        .copy();
    ...
    public static boolean mergeItemStack(List<Slot> slots, ItemStack stack, int start, int length, boolean r,
        boolean limit) {
        ...
        if (stack.stackSize > 0) {
            ...
            while (stack.stackSize > 0 && (!r && i < length || r && i >= start)) {
                ...
                if (slot.isItemValid(stack) && existingStack == null) {
                    int maxStack = limit ? Math.min(stack.getMaxStackSize(), slot.getSlotStackLimit())
