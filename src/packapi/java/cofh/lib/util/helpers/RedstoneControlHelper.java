...
public class RedstoneControlHelper {
    ...
    public static void addRSControlInformation(ItemStack stack, List<String> list) {
        ...
        if (hasRSControl(stack)) {
            switch (stack.stackTagCompound.getByte("RSControl")) {
                case 0:
                    list.add(
                        StringHelper.localize("info.cofh.signal") + ": "
                            + StringHelper.RED
                            + StringHelper.localize("info.cofh.disabled")
                            + StringHelper.END);
                    return;
                case 1:
                    list.add(
    ...
    public static ControlMode getControl(ItemStack stack) {
        ...
        return stack.stackTagCompound == null ? ControlMode.DISABLED
            : ControlMode.values()[stack.stackTagCompound.getByte("RSControl")];
    }
