...
public class SecurityHelper {
    ...
    public static void setup() {
        ...
        EnumConnectionState.PLAY.func_150755_b()
            .put(-26, Login.S__PacketSendUUID.class);
        ...
        FMLCommonHandler.instance()
            .bus()
            .register(new Login.S__PacketSendUUID());
        setup = true;
    }
    ...
    private static UUID getClientId(EntityPlayer player) {
        ...
        if (cachedId == null) {
            cachedId = Minecraft.getMinecraft().thePlayer.getGameProfile()
                .getId();
        }
        return cachedId;
    }
        ...
        if (SecurityHelper.isSecure(stack)) {
            ...
            if (!stack.stackTagCompound.hasKey("Owner") && !hasUUID) {
                ...
                if (hasUUID && stack.stackTagCompound.hasKey("Owner")) {
                    list.add(
                        StringHelper.localize("info.cofh.owner") + ": " + stack.stackTagCompound.getString("Owner"));
                } else {
                    list.add(
                        StringHelper.localize("info.cofh.owner") + ": "
                            + StringHelper.localize("info.cofh.anotherplayer"));
                }
            }
        }
    }
    ...
    public static AccessMode getAccess(ItemStack stack) {
        ...
        return stack.stackTagCompound == null ? AccessMode.PUBLIC
            : AccessMode.values()[stack.stackTagCompound.getByte("Access")];
    }
    ...
    public static GameProfile getProfile(UUID uuid, String name) {
        ...
        GameProfile owner = MinecraftServer.getServer()
            .func_152358_ax()
            .func_152652_a(uuid);
        if (owner == null) {
            GameProfile temp = new GameProfile(uuid, name);
            owner = MinecraftServer.getServer()
                .func_147130_as()
