    ...
    public boolean generate(World world, Random rand, int xStart, int yStart, int zStart) {
        ...
        for (x = 0; x < width; ++x) {
            for (z = 0; z < width; ++z) {
                for (y = 0; y < height; ++y) {
                    ...
                    if (flag) {
                        if (y >= heightOff) {
                            Material material = world.getBlock(xStart + x, yStart + y, zStart + z)
                                .getMaterial();
        ...
        for (x = 0; x < width; ++x) {
            for (z = 0; z < width; ++z) {
                for (y = 0; y < height; ++y) {
                    if (spawnBlock[(x * width + z) * height + y]
                        && world.getBlock(xStart + x, yStart + y - 1, zStart + z)
                            .equals(Blocks.dirt)
                ...
                for (z = 0; z < width; ++z) {
                    for (y = 0; y < height; ++y) {
                        ...
                        if (flag && (solidOutline | y < heightOff || rand.nextInt(2) != 0)
                            && (totalOutline || world.getBlock(xStart + x, yStart + y, zStart + z)
                                .getMaterial()
                                .isSolid())) {
