...
public class WorldGenMassiveTree extends WorldGenerator {
    ...
    public WorldGenMassiveTree(List<WeightedRandomBlock> resource, List<WeightedRandomBlock> leaf,
        List<WeightedRandomBlock> block) {
            ...
            for (int zMod = 0; zMod <= size;) { // negative values handled below
                ...
                if (distSq > maxDistSq) {
                    ...
                    for (t = -1; t <= 1; t += 2) {
                        ...
                        if (safeGrowth
                            ? canGenerateInBlock(worldObj, x, y, z, genBlock)
                                && (!treeChecks || (block.isAir(worldObj, x, y, z) || block.isLeaves(worldObj, x, y, z)
     ...
     * (in blocks) before a non-air, non-leaf
    ...
    public synchronized boolean generate(World world, Random par2Random, int x, int y, int z) {
        ...
        if (!this.validTreeLocation()) {
            ...
            // logger.info(String.format("%s for trunk, %s for leaf nodes, %s for leaves, %s for branches", trunk,
            // nodes, leaves, bases));
    ...
    // private static final org.apache.logging.log4j.Logger logger = org.apache.logging.log4j.LogManager.getLogger("Tree
    // logger");
    ...
    public void setBlockAndNotifyAdequately(World world, int x, int y, int z, Block block, int meta) {
        ...
        x &= 15;
        z &= 15;
        if (block != null && subChunk.getBlockByExtId(x, y & 15, z)
            .hasTileEntity(subChunk.getExtBlockMetadata(x, y & 15, z))) {
