...
public class WorldGenMinableCluster extends WorldGenerator {
        ...
        for (int j = 0, e = mat.length; j < e; ++j) {
            ...
            if ((-1 == genBlock.metadata || genBlock.metadata == world.getBlockMetadata(x, y, z))
                && (block.isReplaceableOreGen(world, x, y, z, genBlock.block)
                    || block.isAssociatedBlock(genBlock.block))) {
                return true;
            }
        }
        return false;
    ...
    public static boolean generateBlock(World world, int x, int y, int z, WeightedRandomBlock[] mat,
        List<WeightedRandomBlock> o) {
